...
void jacobi(double* grid_source, double* grid_target, uint32_t x, uint32_t y) {
	...
	for (uint32_t dy = 1; dy < y - 1; dy++) {
			...
			for (uint32_t dx = 1; dx < x-remainder-1; dx+=4) {
				...
				printf("y: %d x: %d value: %lf\n", dy, dx, grid_target[dy * x + dx]); // debug
				printf("y: %d x: %d value: %lf\n", dy, dx+1, grid_target[dy * x + dx+1]); // debug
				printf("y: %d x: %d value: %lf\n", dy, dx+2, grid_target[dy * x + dx+2]); // debug
				printf("y: %d x: %d value: %lf\n", dy, dx+3, grid_target[dy * x + dx+3]); // debug
			}
			if(remainder >= 1) {
